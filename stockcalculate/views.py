...
#	for x in data:
#	print data["Ethical Investing"]
	return render_to_response("stockcalculate/invest/investStock.html",{'data':data["Ethical Investing"], 'data1':data["Growth Investing"], 'data2':data["Index Investing"], 'data3':data["Quality Investing"], 'data4':data["Value Investing"], 'username':username})
...
def addData(request):
	...
	try:
		...
		dictStock = readJsonFile()[strategy]
		for i in range(0,5):
			n = str(dictStock[i]['name'])
			p = int(dictStock[i]['portion'])
#			print n, p 
			# rjs = readJsonStream(n)
			# print rjs
			#pp = str(getStockPrice(n))data.DataReader(str(x),'yahoo',SevenDaysBefore(day),day)[-5:]si.get_live_price("aapl")
			# print pp
			price = float(p)
			monportion = float(amount) * p / 100
#			share = int(monportion / price)
			dicton.append([])			
			dicton[i].append(n)
			dicton[i].append(monportion)
			dicton[i].append(price)

	except Exception as inst:
#		print "2"
		error=inst
	return render_to_response("stockcalculate/addedStocks.html",{'amount': amount, 'strategy': strategy, 'dicton': dicton }  )
